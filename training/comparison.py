import pandas as pd
import matplotlib.pyplot as plt
import os

# Load result dataframes from each model
result_logistic_regression = pd.read_csv("results/logistic_regression_result.csv")
result_random_forest = pd.read_csv("results/random_forest_result.csv")
result_lg = pd.read_csv("results/lgb_result.csv")
result_xgb = pd.read_csv("results/xgb_result_tuned.csv")
result_XGB=pd.read_csv("results/xgb_result_f15.csv")

lr=os.path.getsize("others/logistic_regression_model.pkl")/(1024 * 1024)
rf=os.path.getsize("others/random_forest_model.pkl")/(1024 * 1024)
lg=os.path.getsize("others/lightgbm_model.pkl")/(1024 * 1024)
xgb=os.path.getsize("others/xgb_model_tuned.pkl")/(1024 * 1024)
xgb_base=os.path.getsize("others/xgb_model.pkl")/(1024 * 1024)
xgb_15=os.path.getsize("models/xgb_model_f15.pkl")/(1024 * 1024)
# Add size of model column
# Concatenate the dataframes
result_combined = pd.concat([result_xgb, result_logistic_regression, result_random_forest, result_lg,result_XGB], ignore_index=True)
result_combined["Size of model (MB)"]=[xgb_base,xgb,lr,rf,lg,xgb_15]
print(result_combined)

plt.bar(result_combined["Model"], result_combined["Accuracy"])
plt.axhline(y=result_combined["Accuracy"].iloc[5], color="r", linestyle="--")
plt.xlabel("Model")
plt.ylabel("Prediction Accuracy (%)")
plt.title("Accuracy Comparison")
plt.show()

plt.bar(result_combined["Model"], result_combined["Macro_f1"])
plt.axhline(y=result_combined["Macro_f1"].iloc[5], color="r", linestyle="--")
plt.xlabel("Model")
plt.ylabel("Models Macro F1 (%)")
plt.title("Macro F1 Comparison")
plt.show()  

plt.bar(result_combined["Model"], result_combined["F1 score"])
plt.axhline(y=result_combined["F1 score"].iloc[5], color="r", linestyle="--")
plt.xlabel("Model")
plt.ylabel("Models F1 Score (%)")
plt.title("F1 Score Comparison")
plt.show()

plt.bar(result_combined["Model"], result_combined["Training time"])
plt.axhline(y=result_combined["Training time"].iloc[5], color="r", linestyle="--")
plt.xlabel("Model")
plt.ylabel("Model Training Time (s)")
plt.title("Model Training Time Comparison")
plt.show()

plt.bar(result_combined["Model"], result_combined["Testing time"])
plt.axhline(y=result_combined["Testing time"].iloc[5], color="r", linestyle="--")
plt.xlabel("Model")
plt.ylabel("Model Testing Time (s)")
plt.title("Model Testing Time Comparison")
plt.show()

plt.bar(result_combined["Model"], result_combined["Size of model (MB)"])
plt.axhline(y=result_combined["Size of model (MB)"].iloc[5], color="r", linestyle="--")
plt.xlabel("Model")
plt.ylabel("Size of model (MB)")
plt.title("Size of model (MB) Comparison")
plt.show()